...
## Status (2026-06-25)
- **Latest push:** `3a9d33d` — 4 opt-in military/federal reviewer agents
  (`usajobs`, `fed_format`, `mil_translate`, `mp_security`; reviewer lenses only,
  no synth/export federal-format output). Pushed to
  `trendtubedev-lab/resume-builder` main.
  Prior pushes: `e7fa9a8` (clickable contact hyperlinks in PDF + DOCX:
  `_classify_contact`/`_canvas_contact`/`_add_hyperlink`),
  `23cbb6c` (export professionalism pass: PDF bullet fix +
...
- **19 reviewer personas** in pool: original 4 (default on) + 15 opt-in. Opt-in set: Engineering Bar-Raiser, Leveling Calibrator, Career Narrative Strategist, Transferable Skills Interpreter, Attention & Perception Specialist, Domain Credentialing Auditor, Industry Format & Culture Fit Auditor, Competitive Field Analyst, Executive Presence Assessor, Commitment Signal Analyst, First-Impression Clarity Analyst, **+ military/federal (added 2026-06-25): USAJOBS Federal Application Specialist (`usajobs`), Federal Resume Format Auditor (`fed_format`), Military-to-Civilian Translator (`mil_translate`), Security/MP & Clearance Specialist (`mp_security`)**. Military/federal agents are reviewer lenses only — no synth/export federal-format output (deferred follow-up). Personas are dynamic end-to-end: `/api/personas` enumerates `get_personas()`, UI default-selects only `default_enabled` — adding a preset needs NO route/UI change.
